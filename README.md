...
Esto levanta ambos procesos simultáneamente:
- **API REST** → http://localhost:8000  (`uvicorn`, puerto 8000)
- **Dashboard** → http://localhost:8501  (Streamlit, puerto 8501)

Presiona `Ctrl+C` para detener los dos a la vez.
...
- Resumen numérico: total / pendientes / completadas
- Tabla de tareas con colores por estado (amarillo = pendiente, verde = completada)
- Filtro por estado (todas / pendientes / completadas) en el sidebar
- Formulario de creación en el sidebar (título obligatorio, descripción opcional)
- Botón ✔ para marcar como completada (solo en tareas pendientes)
- Botón 🗑 para eliminar cualquier tarea
- Toda comunicación via HTTP a la API — sin acceso directo a la DB
